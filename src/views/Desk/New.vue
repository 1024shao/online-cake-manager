  ...
  <div class="new">
    ...
    <div class="main">
      <div class="cake" v-for="item in newList" :key="item.id">
        <img :src="item.image1" class="back" @click="goDetail(item.id)" />
...
import { reqNewGoods, reqAddToShopCart } from '../../api/index.js'
export default defineComponent({
  setup() {
    ...
    onMounted(() => {
      reqNewGoods({ whichPage: state.whichPage, eachPageSize: state.eachPageSize }).then(res => {
        state.newList = res.data.response.data
      })
    })
    // 加入到购物车
